...
> - 所有已配置仓库中的代码都可能被 AI 自动修改、提交、推送，请使用你愿意仔细审阅的专属仓库。
...
1. 你提交一个任务描述（"给用户 API 添加输入校验"）并选择目标仓库
2. Dev Assistant 在该仓库中创建新分支 + git worktree
...
先启动服务（第 5 步），然后打开 **http://localhost:8089/settings.html**，在界面中添加仓库并配置其他选项。

Dev Assistant 支持**多仓库**——每个仓库拥有独立的任务、定时任务和 MR/PR。在 Settings 页面按需添加多个仓库。

> **请慎重选择仓库。** Claude 会在这些仓库中自主创建分支、提交、推送。选你自己拥有或有明确授权修改的项目，并确保它们的远端是你愿意接收自动生成 MR/PR 的地方。
...
- 在任务进入 COMMITTING 之前手动取消，然后到 `<仓库路径>/.worktrees/task-<id>/` 查看代码
...
| `GLAB_CONFIG_DIR` | 否 | `.gitlab/` | 全局默认的 glab 配置目录 |
| `GH_CONFIG_DIR` | 否 | `.github/` | 全局默认的 gh 配置目录 |

### 仓库设置

通过 **Settings → Repositories** 或 `POST /repos` API 管理仓库。每个仓库可配置：

| 字段 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `name` | 是 | — | 仓库显示名称 |
| `path` | 是 | — | git 仓库绝对路径 |
| `default_branch` | 否 | `master` | 默认分支名（`main`、`master` 等） |
| `glab_config_dir` | 否 | *（回退到全局设置）* | 仓库专属 glab 配置目录 |
| `gh_config_dir` | 否 | *（回退到全局设置）* | 仓库专属 gh 配置目录 |
...
├── config.json                 # 全局设置 + 仓库列表（通过 /settings.html 编辑）
└── repos/
    └── <repo-id>/
        ├── dev-tasks.json      # 该仓库的任务数据库
        ├── cron-tasks.json     # 该仓库的 Cron 任务定义
...
**`manage.sh start` 提示"No repositories configured"**

打开 `http://localhost:8089/settings.html`，在 Repositories 部分至少添加一个仓库。
...
- 仓库没有 `master` 分支 → 在 Settings 中将该仓库的 `default_branch` 设置为 `main`
...
- 在已配置的仓库中启动 `claude` 子进程，授予广泛的工具权限（`Bash`、`Write`、`Edit` 等）
...
- 使用专门的仓库，做好被 AI 改动的心理准备。不要指向生产代码库。
